...
- Извлечение WAV остаётся без изменений.
...
Статус в новом проекте:
- Whisper заменяется на **faster-whisper** (CTranslate2 backend) — тот же API, но 4x быстрее и меньше памяти
- Язык становится настраиваемым (по умолчанию `ru`)
- Параметр `CHUNK_SECONDS` остаётся 300, но может быть сделан настраиваемым
- Код переносится в `backend/pipeline.py`
...
**Почему средняя точка, а не overlap?**
Простота и скорость. Overlap-based подход (какой спикер занимает большую часть сегмента) дал бы чуть лучшую точность на границах, но средняя точка достаточно точна для совещаний, где спикеры редко перебивают друг друга.

Статус в новом проекте: Переносится без изменений. Возможно улучшение до overlap-based в будущем.
...
|-------------|---------------------|-----------|
| `worker.py:30-50` (FFmpeg WAV) | `backend/pipeline.py` | Добавляется предварительная конвертация в MP3 |
| `worker.py:64-70` (HF auth) | `backend/server.py` lifespan | Выносится из pipeline в инициализацию сервера |
| `worker.py:72-85` (диаризация) | `backend/pipeline.py` | Становится асинхронным (asyncio.to_thread) |
| `worker.py:88-126` (транскрибация) | `backend/pipeline.py` | Whisper → faster-whisper, язык параметризуется |
| `worker.py:128-137` (alignment) | `backend/pipeline.py` | Без изменений |
| `worker.py:140-148` (форматирование) | `backend/formatters.py` | Добавляется DOCX, убирается MD |
...
│   │   pagination, popover, radio-group, resizable, select,
...
Использование в WhisperAnnote (этап 2+, не блокирующее):
...
- **Чанкинг по 300 секунд** — эмпирически выверенный размер для баланса память/качество
- **Midpoint alignment** — простое и эффективное сопоставление спикеров с сегментами
